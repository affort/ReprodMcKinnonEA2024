...
```{r Rejection interval based on null hypothesis}
...
The rejection region is when the coefficient for year is smaller than 
`r rejection[1]` or greater than `r rejection[2]`.

```{r Reproduction: mean and variance}
...
Results are identical to Fig. S2(d).
...
Results are identical to Fig. S2(e).

```{r New test case for quadratic}
...
The power of the test remains high for quadratically-increasing variance.

``` {r New test case for exponential}
###########################################
